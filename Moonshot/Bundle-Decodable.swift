...
extension Bundle {
    func decode<T: Codable>(_ file: String) -> T {
        ...
        do {
            ...
            fatalError(
                "Failed to decode \(file) from bundler due to type missmatch - \(context.debugDescription)"
            )
        } catch DecodingError.valueNotFound(let type, let context) {
            fatalError(
                "Failed to decode \(file) from bunlde due to missing value of type '\(type)' - \(context.debugDescription)"
            )
